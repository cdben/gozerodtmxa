...
type (
	UserAccountModel interface {
		...
		AdjustBalance(db *sql.DB, userId int64, amount int64) (int64, error)
	}
...
func (m *defaultUserAccountModel) AdjustBalance(db *sql.DB, userId int64, amount int64) (int64, error) {
	/*
		...
		return m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
			...
			return db.Exec(query, amount, amount, userId)
		}, IdKey)
	*/
	query := fmt.Sprintf("update %s set balance=balance+? where balance >= -? and user_id=?", m.table)
	return dtmimp.DBExec(dtmimp.DBTypeMysql, db, query, amount, amount, userId)
}
